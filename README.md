...
|----------|---------|-------------|
| `/app-pagebank/api/PageCache` | GET, POST | List/create cache entries |
| `/app-pagebank/api/PageCache/{url}` | GET, PUT, DELETE | Read/update/delete a cache entry |
...
MCP tools for PageCache table operations are auto-generated from the `@export` schema. Any MCP-compatible agent (Claude Code, Cursor, Windsurf) can discover and use them via the standard MCP protocol at `POST /app-pagebank/api/mcp`.
